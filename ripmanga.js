...
function scrapechapters() {
	...
	if (MANGAHERE_URL.indexOf("m.mangahere") !=-1) MANGAHERE_URL = MANGAHERE_URL.replace("m.mangahere", "www.mangahere");
...
function updatenexturl(currentnexturl) {
	$.YQL("select * from html where url='" + currentnexturl + "'", function(data) {
		console.log(JSON.stringify(data.query.results.body.section[1].a.img[1].src));
		producedlink += JSON.stringify(data.query.results.body.section[1].a.img[1].src).substring(1, JSON.stringify(data.query.results.body.section[1].a.img[1].src).length - 1);
		if (JSON.stringify(data.query.results.body.section[1].a.href).substring(1, JSON.stringify(data.query.results.body.section[1].a.href).length - 1) != "javascript:void(0);") {
			...
			getShortURL(producedlink, function(shortURL) {
				mangapage.innerHTML = "<a href=\"http://FreeHTMLtoPDF.com/?convert=" + escape(shortURL) + "&size=A4\">Download Manga</a>";
			});
		}
	});
}
